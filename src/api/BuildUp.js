...
// 极速组装单列表
export function buildGetList(query) {
  var params = new URLSearchParams()
...
export function buildlist(query) {
  ...
  if (query.buildupId !== '' && query.buildupId !== null && query.buildupId !== undefined) {
    params.append('buildupId', query.buildupId) // 你要传给后台的参数值 key/value
  }
  if (query.buildupNumber !== '' && query.buildupNumber !== null && query.buildupNumber !== undefined) {
    params.append('buildupNumber', query.buildupNumber) // 你要传给后台的参数值 key/value
  }
...
// 删除组装单
export function deletebuildup(query, query2) {
  var params = new URLSearchParams()
