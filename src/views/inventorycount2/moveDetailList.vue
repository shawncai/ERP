    ...
    <el-card :body-style="	{ padding: '10px' }" class="box-card" shadow="never">
      ...
      <el-table
        ...
        <el-table-column
          ...
          fixed
          align="center"/>
        <el-table-column :label="$t('update4.receiptNumber')" :resizable="false" sortable align="center" min-width="100" fixed>
...
export default {
  ...
  methods: {
    ...
    handleDetail(row) {
      console.log('row========>', row)
      const param = {}
      param.receiptNumber = row.receiptNumber
    ...
    handleExport() {
      ...
      this.downloadLoading = true
        import('@/vendor/Export2Excel').then(excel => {
          const tHeader = ['单据日期', '单据编号', '物品编码', '物品名称', '规格型号', '单位', '调拨数量', '调入仓库', '调出仓库']
          const filterVal = ['receiptDate', 'receiptNumber', 'productCode', 'productName', 'productType', 'unit', 'quantity', 'moveInRepositoryName', 'moveOutRepositoryName']
