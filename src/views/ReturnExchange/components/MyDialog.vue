<template>
  <el-dialog :visible.sync="editVisible" :editcontrol="editcontrol" :editdata="editdata" :close-on-press-escape="false" :title="personalForm.exchangeNumber +'    修改'" width="1010px" class="edit" top="-10px" @close="$emit('update:editcontrol', false)">
    <!--基本信息-->
...
export default {
  ...
  methods: {
    ...
    getdatatime() { // 默认显示今天
      ...
      this.personalForm.exchangeDate = date.getFullYear() + '-' + this.getzero(date.getMonth() + 1) + '-' + this.getzero(date.getDate())
      console.log(this.personalForm.exchangeDate)
    },
    // 配送员focus事件
    handlechooseStock() {
