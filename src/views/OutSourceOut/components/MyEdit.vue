<template>
  <el-dialog :visible.sync="editVisible" :editcontrol="editcontrol" :editdata="editdata" :close-on-press-escape="false" :title="$t('OutSource.xgsss')" class="edit" width="1010px" top="-30px" @close="$emit('update:editcontrol', false)">
    <!--基本信息-->
          ...
          <el-row>
            ...
            <el-col :span="6">
              <el-form-item :label="$t('SaleReturn.sourceNumber')" style="width: 100%;">
                <el-input v-model="personalForm.sourceNumber" :disabled="IsNumber" style="margin-left: 18px;width: 150" @focus="openoppo"/>
...
import { updateOutsourceOut } from '@/api/OutSourceOut'
...
var _that
export default {
  components: { MyRepository, MyCreate, MyAccept, MyDetail, OutSource },
  props: {
    editcontrol: {
      type: Boolean,
      default: false
  ...
  watch: {
    ...
    editdata() {
      ...
      console.log('this.personalForm.outsourceOutDetailVos', this.personalForm.outsourceOutDetailVos)
      ...
      for (const i in this.list2) {
        ...
        this.list2[i].location = this.list2[i].locationCode
  ...
  methods: {
    ...
    handleEditok() {
      ...
      updateOutsourceOut(parms, parms2).then(res => {
        if (res.data.ret === 200) {
          this.$notify({
