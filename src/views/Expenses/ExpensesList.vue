<template>
  <div class="ERP-container">
    ...
    <el-card :body-style=" { padding: '6px'}" class="box-card" shadow="never">
      ...
      <!-- 新建操作 -->
      <el-button v-permission="['266-92-1']" v-waves class="filter-item2" size="small" icon="el-icon-plus" type="success" style="width: 86px" @click="handleAdd">{{ $t('public.add') }}</el-button>
    </el-card>
...
export default {
  ...
  methods: {
    ...
    async setvoucherdata(val) {
      ...
      this.selectid = val.expensesDetailVos.map(item => {
        return {
          id: item.id
        }
      })
      if (voucherdata) {
        ...
        const testarr = val.expensesDetailVos
