<template>
  <div class="ERP-container">
    <el-card class="box-card" style="margin-top: 10px" shadow="never">
      <el-input v-model="getemplist.title" :placeholder="$t('SaleOrder.title')" class="filter-item" clearable @keyup.enter.native="handleFilter"/>
      ...
      <el-select v-model="getemplist.sendType" :placeholder="$t('SaleOrder.sendType')" :value="getemplist.sendType" class="filter-item" clearable @keyup.enter.native="handleFilter">
    ...
    <el-card class="box-card" style="margin-top: 10px" shadow="never">
      ...
      <!-- 表格导出操作 -->
      <el-button v-permission="['54-57-6']" v-waves :loading="downloadLoading" class="filter-item" style="width: 86px" @click="handleExport"> <svg-icon icon-class="daochu"/>{{ $t('public.export') }}</el-button>
      <!-- 打印操作 -->
      <el-button v-permission="['54-57-7']" v-waves class="filter-item" icon="el-icon-printer" style="width: 86px" @click="handlePrint">{{ $t('public.print') }}</el-button>
      <!-- 新建操作 -->
      <el-button v-permission="['54-57-1']" v-waves class="filter-item" icon="el-icon-plus" type="success" style="width: 86px" @click="handleAdd">{{ $t('public.add') }}</el-button>
    </el-card>
    ...
    <el-card class="box-card" style="margin-top: 10px" shadow="never">
      ...
      <el-table
        ...
        <el-table-column :label="$t('public.actions')" :resizable="false" align="center" min-width="540">
          <template slot-scope="scope">
            <el-button v-permission2="['54-57-3', scope.row.createPersonId]" v-show="scope.row.judgeStat === 0" :title="$t('updates.xg')" type="primary" size="mini" icon="el-icon-edit" circle @click="handleEdit(scope.row)"/>
...
<script>
import { checkReceiptOrder2 } from '@/api/public'
