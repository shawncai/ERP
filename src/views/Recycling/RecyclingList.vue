...
<script>
import { searchrecycling, updaterecycling2, deleterecycling, recyclinggetList } from '@/api/Recycling'
...
export default {
  ...
  methods: {
    ...
    getlist() {
      ...
      recyclinggetList(this.getemplist).then(res => {
        if (res.data.ret === 200) {
    ...
    handleEdit(row) {
      ...
      searchrecycling(parms).then(res => {
        if (res.data.ret === 200) {
          ...
          if (this.personalForm.sexId !== null && this.personalForm.sexId !== '' && this.personalForm.sexId !== undefined) {
            this.personalForm.sexId = String(res.data.data.content.list[0].sexId)
          }
        }
      })
    },
