<template>
  <div class="ERP-container">
    ...
    <el-card :body-style=" { padding: '6px'}" class="box-card" shadow="never">
      ...
      <!-- 表格导出操作 -->
      <el-button v-permission="['131-278-6']" v-waves :loading="downloadLoading" class="filter-item2" size="small" style="width: 86px" @click="handleExport"> <svg-icon icon-class="daochu"/>{{ $t('public.export') }}</el-button>
      <!-- 打印操作 -->
      <el-button v-permission="['131-278-7']" v-waves class="filter-item2" size="small" icon="el-icon-printer" style="width: 86px" @click="handlePrint">{{ $t('public.print') }}</el-button>
      <!-- 新建操作 -->
      <el-button v-permission="['131-278-1']" v-waves class="filter-item2" size="small" icon="el-icon-plus" type="success" style="width: 86px" @click="handleAdd">{{ $t('public.add') }}</el-button>
      ...
      <el-table
        ...
        <el-table-column :label="$t('public.actions')" :resizable="false" align="center" min-width="230">
          <template slot-scope="scope">
            ...
            <el-button v-permission="['131-278-2']" v-if="scope.row.isEffective === 1" size="mini" type="danger" @click="handledisable(scope.row)">{{ $t('public.disable') }}</el-button>
            <el-button v-permission="['131-278-2']" v-else size="mini" type="success" @click="handleenable(scope.row)">{{ $t('public.enable') }}</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 列表结束 -->
...
import { addCostCategory, updateCostCategory, searchCostCategory } from '@/api/categorymanage'
...
export default {
  ...
  methods: {
    ...
    handledisable(row) {
      ...
      this.editCategoryForm.iseffective = '2'
    ...
    handleenable(row) {
      this.editCategoryForm = Object.assign({}, row)
      this.editCategoryForm.iseffective = '1'
      updateCostCategory(this.editCategoryForm).then(res => {
        if (res.data.ret === 200) {
          this.$notify({
            title: 'successful',
    ...
    getlist() {
      ...
      searchCostCategory(this.getemplist).then(res => {
        if (res.data.ret === 200) {
