<template>
  <div class="ERP-container">
    <el-card :body-style="	{ padding: '5px' }" class="box-card" style="margin-top: 5px" shadow="never">
      ...
      <el-input v-model="getemplist.carname" :placeholder="$t('LogisticsCar.carname')" size="small" class="filter-item" clearable @keyup.enter.native="handleFilter"/>

      <el-input v-model="getemplist.carnumber" :placeholder="$t('LogisticsCar.carnumber')" size="small" class="filter-item" clearable @keyup.enter.native="handleFilter"/>

      <el-input v-model="getemplist.licencenumber" :placeholder="$t('LogisticsCar.licencenumber')" size="small" class="filter-item" clearable @keyup.enter.native="handleFilter"/>
      ...
      <el-popover
        ...
        size="small"
        trigger="click">
        <el-select v-model="getemplist.cartype" size="small" placeholder="车辆类型" style="width: 40%;float: left;margin-left: 20px" clearable>
          <el-option value="1" label="大货车"/>
          <el-option value="2" label="小货车"/>
          <el-option value="3" label="小轿车"/>
        </el-select>
        <el-select v-model="getemplist.stat" size="small" placeholder="车辆状态" style="width: 40%;float: right;margin-right: 20px" clearable>
    ...
    <el-card :body-style=" { padding: '6px'}" class="box-card" shadow="never">
      ...
      <!-- 表格导出操作 -->
      <el-button v-permission="['1-247-249-6']" v-waves :loading="downloadLoading" size="small" class="filter-item2" style="width: 86px" @click="handleExport"> <svg-icon icon-class="daochu"/>{{ $t('public.export') }}</el-button>
      <!-- 打印操作 -->
      <!-- <el-button v-permission="['1-247-249-7']" v-waves size="small" class="filter-item2" icon="el-icon-printer" style="width: 86px" @click="handlePrint">{{ $t('public.print') }}</el-button> -->
      <!-- 新建操作 -->
      <el-button v-permission="['1-247-249-1']" v-waves size="small" class="filter-item2" icon="el-icon-plus" type="success" style="width: 86px" @click="handleAdd">{{ $t('public.add') }}</el-button>
    </el-card>
    <el-card :body-style="	{ padding: '10px' }" class="box-card" shadow="never">
      ...
      <el-table
        ...
        <el-table-column
          ...
          fixed="left"
          align="center"/>
        <el-table-column :label="$t('LogisticsCar.carnumber')" :resizable="false" fixed="left" align="center" min-width="150">
        ...
        <el-table-column :label="$t('LogisticsCar.carname')" :resizable="false" fixed="left" align="center" width="200">
          <template slot-scope="scope">
  ...
  methods: {
    ...
    handleCommand(command) {
      ...
      if (command === 'delete') {
        this.$confirm(this.$t('prompt.scts'), this.$t('prompt.ts'), {
          ...
          deletecar(ids, this.$store.getters.userId).then(res => {
    ...
    handleDelete(row) {
      this.$confirm(this.$t('prompt.scts'), this.$t('prompt.ts'), {
        ...
        deletecar(row.id, this.$store.getters.userId).then(res => {
          if (res.data.ret === 200 || res.data.ret === 100) {
            this.$notify({
              title: this.$t('prompt.sccg'),
              type: 'success',
