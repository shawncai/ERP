    ...
    <el-card class="box-card" style="margin-top: 15px" shadow="never">
      ...
      <div class="container">
        <el-editable
          ...
          <el-editable-column :label="$t('accessComponent.accessQuantity')" prop="accessQuantity" align="center" width="150px"/>
          <el-editable-column :edit-render="{name: 'ElInputNumber', attrs: {min: 1.00, precision: 2}, type: 'visible'}" :label="$t('accessComponent.returnQuantity')" prop="outQuantity" align="center" min-width="150">
            <template slot="edit" slot-scope="scope">
              <el-input-number
                :precision="2"
                :controls="true"
...
<script>
import { updateReturnComponent } from '@/api/AccessComponent'
import { getdeptlist } from '@/api/BasicSettings'
import { searchStockCategory } from '@/api/StockCategory'
import { searchSaleCategory } from '@/api/SaleCategory'
import { searchCategory } from '@/api/Supplier'
import { ratelist } from '@/api/Installmentrate'
import { locationlist, getlocation6, getlocation, batchlist } from '@/api/public'
  ...
  methods: {
    ...
    handleEditok() {
      ...
      if (EnterDetail.length !== 0) {
        ...
        const parms = this.personalForm.id
        updateReturnComponent(parms, parms2).then(res => {
          if (res.data.ret === 200) {
            this.$notify({
