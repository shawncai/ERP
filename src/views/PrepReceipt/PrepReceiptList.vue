<template>
  <div class="ERP-container">
    <el-card :body-style="	{ padding: '5px' }" class="box-card" style="margin-top: 5px" shadow="never">
      ...
      <el-input v-model="getemplist.title" :placeholder="$t('PrepReceipt.title')" size="small" class="filter-item" clearable @keyup.enter.native="handleFilter"/>

      <el-input v-model="getemplist.number" :placeholder="$t('updates.yskddh')" size="small" class="filter-item" clearable @keyup.enter.native="handleFilter"/>

      <el-input v-model="handlePersonId" :placeholder="$t('updates.hsr')" size="small" class="filter-item" clearable @clear="restFilter" @keyup.enter.native="handleFilter" @focus="handlechooseStock"/>
    ...
    <el-card :body-style=" { padding: '6px'}" class="box-card" shadow="never">
      ...
      <!-- 表格导出操作 -->
      <el-button v-permission="['266-98-6']" v-waves :loading="downloadLoading" class="filter-item2" size="small" style="width: 86px" @click="handleExport"> <svg-icon icon-class="daochu"/>{{ $t('public.export') }}</el-button>
      <!-- 打印操作 -->
      <el-button v-permission="['266-98-7']" v-waves class="filter-item2" size="small" icon="el-icon-printer" style="width: 86px" @click="handlePrint">{{ $t('public.print') }}</el-button>
      <!-- 新建操作 -->
      <el-button v-permission="['266-98-1']" v-waves class="filter-item2" size="small" icon="el-icon-plus" type="success" style="width: 86px" @click="handleAdd">{{ $t('public.add') }}</el-button>
    </el-card>
    ...
    <el-card :body-style="	{ padding: '10px' }" class="box-card" shadow="never">
      ...
      <el-table
        ...
        <el-table-column :label="$t('PrepReceipt.receiptMoney')" :resizable="false" prop="receiptMoney" align="center" min-width="150">
          <template slot-scope="scope">
...
import { searchprepReceipt, updateprepReceipt2, deleteprepReceipt, searchprepReceiptGetList } from '@/api/PrepReceipt'
...
export default {
  ...
  methods: {
    ...
    getlist() {
      ...
      searchprepReceiptGetList(this.getemplist).then(res => {
        if (res.data.ret === 200) {
    ...
    handleEdit(row) {
      ...
      searchprepReceipt(parms).then(res => {
        if (res.data.ret === 200) {
          ...
          if (this.personalForm.accountType !== null && this.personalForm.accountType !== '' && this.personalForm.accountType !== undefined) {
            this.personalForm.accountType = String(res.data.data.content.list[0].accountType)
          }
