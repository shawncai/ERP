<template>
  <div class="ERP-container">
    <el-card class="box-card" style="margin-top: 10px" shadow="never">
      ...
      <el-input v-model="getemplist.title" :placeholder="$t('PrepReceipt.title')" class="filter-item" clearable @keyup.enter.native="handleFilter"/>

      <el-input v-model="getemplist.number" :placeholder="$t('updates.yskddh')" class="filter-item" clearable @keyup.enter.native="handleFilter"/>

      <el-input v-model="handlePersonId" :placeholder="$t('updates.hsr')" class="filter-item" clearable @clear="restFilter" @keyup.enter.native="handleFilter" @focus="handlechooseStock"/>
    ...
    <el-card class="box-card" style="margin-top: 10px" shadow="never">
      ...
      <!-- <el-dropdown @command="handleCommand">
        ...
        <el-dropdown-menu slot="dropdown" style="width: 140px">
          <el-dropdown-item v-permission="['266-98-2']" style="text-align: left" command="delete"><svg-icon icon-class="shanchu" style="width: 40px"/>{{ $t('public.delete') }}</el-dropdown-item>
        </el-dropdown-menu>
      </el-dropdown> -->
      ...
      <!-- 表格导出操作 -->
      <el-button v-permission="['266-98-6']" v-waves :loading="downloadLoading" class="filter-item" style="width: 86px" @click="handleExport"> <svg-icon icon-class="daochu"/>{{ $t('public.export') }}</el-button>
      <!-- 打印操作 -->
      <el-button v-permission="['266-98-7']" v-waves class="filter-item" icon="el-icon-printer" style="width: 86px" @click="handlePrint">{{ $t('public.print') }}</el-button>
      <!-- 新建操作 -->
      <el-button v-permission="['266-98-1']" v-waves class="filter-item" icon="el-icon-plus" type="success" style="width: 86px" @click="handleAdd">{{ $t('public.add') }}</el-button>
    </el-card>
    ...
    <el-card class="box-card" style="margin-top: 10px" shadow="never">
      ...
      <el-table
        ...
        <el-table-column :label="$t('PrepReceipt.title')" :resizable="false" fixed="left" align="center" min-width="150">
          <template slot-scope="scope">
        ...
        <el-table-column :label="$t('PrepReceipt.customerName')" :resizable="false" align="center" min-width="150">
          <template slot-scope="scope">
            <span>{{ scope.row.customerName }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$t('PrepReceipt.closeType')" :resizable="false" align="center" min-width="150">
          <template slot-scope="scope">
            <span>{{ scope.row.closeTypeName }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$t('PrepReceipt.receiptPersonId')" :resizable="false" align="center" min-width="150">
          <template slot-scope="scope">
        ...
        <el-table-column :label="$t('public.actions')" :resizable="false" align="center" min-width="230">
          <template slot-scope="scope">
            <el-button v-permission2="['266-98-3', scope.row.createPersonId]" v-show="scope.row.judgeStat === 0" :title="$t('updates.xg')" type="primary" size="mini" icon="el-icon-edit" circle @click="handleEdit(scope.row)"/>
            ...
            <el-button v-permission="['266-98-76']" v-show="isReview4(scope.row)" :title="$t('updates.fsp')" type="warning" size="mini" circle @click="handleReview4(scope.row)"><svg-icon icon-class="fanhui"/></el-button>
            <el-button v-permission="['266-98-16']" v-show="isReview2(scope.row)" :title="$t('updates.jd')" type="success" size="mini" icon="el-icon-check" circle @click="handleReview2(scope.row)"/>
            <el-button v-permission="['266-98-17']" v-show="isReview3(scope.row)" :title="$t('updates.fjd')" type="success" size="mini" icon="el-icon-back" circle @click="handleReview3(scope.row)"/>
            <el-button v-permission2="['266-98-2', scope.row.createPersonId]" v-show="scope.row.judgeStat === 0" :title="$t('updates.sc')" size="mini" type="danger" icon="el-icon-delete" circle @click="handleDelete(scope.row)"/>
...
<script>
import { voucherlist, addAdvanceVoucher } from '@/api/voucher'
import { searchprepReceipt, updateprepReceipt2, deleteprepReceipt } from '@/api/PrepReceipt'
...
export default {
  ...
  methods: {
      ...
      voucherlist(voucherparms).then(res => {
        if (res.data.ret === 200) {
          if (res.data.data.content.length !== 0) {
            ...
            addAdvanceVoucher(voucherids).then(res => {
              if (res.data.ret === 200) {
                that.$message({
                  type: 'success',
    ...
    getlist() {
      ...
      searchprepReceipt(this.getemplist).then(res => {
        if (res.data.ret === 200) {
    ...
    handleCommand(command) {
      ...
      if (command === 'delete') {
        this.$confirm(this.$t('prompt.scts'), this.$t('prompt.ts'), {
          ...
          deleteprepReceipt(ids, this.$store.getters.userId).then(res => {
    ...
    handleDelete(row) {
      this.$confirm(this.$t('prompt.scts'), this.$t('prompt.ts'), {
        ...
        deleteprepReceipt(row.id, this.$store.getters.userId).then(res => {
          if (res.data.ret === 200 || res.data.ret === 100) {
            this.$notify({
              title: this.$t('prompt.sccg'),
              type: 'success',
    ...
    handleExport() {
      this.downloadLoading = true
        import('@/vendor/Export2Excel').then(excel => {
          ...
          const filterVal = ['id', 'PrepReceiptName', 'PrepReceiptShortName', 'typeName', 'regionName', 'buyerName', 'levelName', 'createName', 'createTime']
