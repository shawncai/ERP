<template>
  <div class="ERP-container">
    <el-card :body-style="	{ padding: '5px' }" class="box-card" style="margin-top: 5px" shadow="never">
      ...
      <el-select v-model="getemplist.stat" :placeholder="$t('updates.stat')" :value="getemplist.stat" size="small" class="filter-item" clearable @keyup.enter.native="handleFilter">
    ...
    <el-card :body-style=" { padding: '6px'}" class="box-card" shadow="never">
      ...
      <!-- 表格导出操作 -->
      <el-button v-permission="['200-250-252-6']" v-waves :loading="downloadLoading" size="small" class="filter-item2" style="width: 86px" @click="handleExport"> <svg-icon icon-class="daochu"/>{{ $t('public.export') }}</el-button>
      <!-- 打印操作 -->
      <!-- <el-button v-permission="['200-250-252-7']" v-waves size="small" class="filter-item2" icon="el-icon-printer" style="width: 86px" @click="handlePrint">{{ $t('public.print') }}</el-button> -->
      <!-- 新建操作 -->
      <el-button v-permission="['200-250-252-1']" v-waves size="small" class="filter-item2" icon="el-icon-plus" type="success" style="width: 86px" @click="handleAdd">{{ $t('public.add') }}</el-button>
      <el-dialog :visible.sync="categoryVisible" title="新建滞纳金规则设置" class="normal" width="600px" center>
        <el-form ref="addCategoryForm" :rules="addCategoryFormRules" :model="addCategoryForm" class="demo-ruleForm" style="margin: 0 auto; width: 400px">
          <el-form-item :label="$t('InstallmentrateList.overdueDay')" label-width="120px" prop="overdueDay">
            <el-input v-model="addCategoryForm.overdueDay" autocomplete="off"/>
          </el-form-item>
          <el-form-item :label="$t('InstallmentrateList.rate2')" label-width="120px" prop="rate">
            <el-input v-model="addCategoryForm.rate" autocomplete="off">
              <template slot="prepend">本期待还金额的</template>
    ...
    <el-card :body-style="	{ padding: '10px' }" class="box-card" shadow="never">
      ...
      <el-table
        ...
        <el-table-column :label="$t('InstallmentrateList.overdueDay')" :resizable="false" align="center" min-width="250">
          <template slot-scope="scope">
        ...
        <el-table-column :label="$t('public.actions')" :resizable="false" align="center" min-width="230">
          <template slot-scope="scope">
            <el-button v-permission="['200-250-252-3']" :title="$t('updates.xg')" type="primary" size="mini" icon="el-icon-edit" circle @click="handleEdit(scope.row)" />
            <el-button v-permission="['200-250-252-3']" :title="$t('updates.sc')" size="mini" type="danger" icon="el-icon-delete" circle @click="handleDelete(scope.row)"/>
          </template>
        </el-table-column>
      </el-table>
      <!-- 列表结束 -->
...
<script>
import { rulelist, addOverdueFineRules, updateOverdueFineRules, deleteOverdueFineRules } from '@/api/Installmentrate'
...
export default {
  ...
  data() {
    return {
      ...
      addCategoryFormRules: {
        ...
        rate: [
          { required: true, message: '请输入滞纳金比例', trigger: 'blur' }
        ],
        isEffective: [
  ...
  methods: {
    ...
    getlist() {
      ...
      rulelist(this.getemplist).then(res => {
        if (res.data.ret === 200) {
    ...
    handleCommand(command) {
      ...
      if (command === 'delete') {
        this.$confirm(this.$t('prompt.scts'), this.$t('prompt.ts'), {
          ...
          deleteOverdueFineRules(ids, this.$store.getters.userId).then(res => {
    ...
    handleDelete(row) {
      this.$confirm('此操作将永久删除该记录, 是否继续?', this.$t('prompt.ts'), {
        ...
        deleteOverdueFineRules(row.id, this.$store.getters.userId).then(res => {
          if (res.data.ret === 200 || res.data.ret === 100) {
            this.$notify({
              title: this.$t('prompt.sccg'),
              type: 'success',
    ...
    handleOk() {
      ...
      this.$refs.editCategoryForm.validate((valid) => {
        if (valid) {
          updateOverdueFineRules(parms).then(res => {
            if (res.data.ret === 200) {
              this.$notify({
                title: 'successful',
                message: '修改成功',
    ...
    handlesave() {
      ...
      this.$refs.addCategoryForm.validate((valid) => {
        if (valid) {
          addOverdueFineRules(parms).then(res => {
            if (res.data.ret === 200) {
              this.$notify({
                title: 'successful',
                message: '新建成功',
