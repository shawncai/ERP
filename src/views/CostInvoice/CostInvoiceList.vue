<template>
  <div class="ERP-container">
    <el-card :body-style="	{ padding: '5px' }" class="box-card" style="margin-top: 5px" shadow="never">
      ...
      <el-input v-model="supplierId" :placeholder="$t('updates.gys')" size="small" class="filter-item" clearable @focus="handlechoose" @clear="restFilter"/>
      <my-supplier :control.sync="empcontrol" @supplierName="supplierName"/>

      <el-input v-model="getemplist.invoiceNumber" :placeholder="$t('updates.hph')" size="small" class="filter-item" clearable @keyup.enter.native="handleFilter"/>
      ...
      <el-select v-model="getemplist.isInclude" :placeholder="$t('CostInvoice.isInclude')" size="small" class="filter-item" clearable style="margin-top: 2%;height: 20px">
    ...
    <el-card :body-style=" { padding: '6px'}" class="box-card" shadow="never">
      ...
      <!-- 表格导出操作 -->
      <el-button v-permission="['266-259-6']" v-waves :loading="downloadLoading" class="filter-item2" size="small" style="width: 86px" @click="handleExport"> <svg-icon icon-class="daochu"/>{{ $t('public.export') }}</el-button>
      <!-- 打印操作 -->
      <el-button v-permission="['266-259-7']" v-waves class="filter-item2" size="small" icon="el-icon-printer" style="width: 86px" @click="handlePrint">{{ $t('public.print') }}</el-button>
      <!-- 新建操作 -->
      <el-button v-permission="['266-259-1']" v-waves class="filter-item2" size="small" icon="el-icon-plus" type="success" style="width: 86px" @click="handleAdd">{{ $t('public.add') }}</el-button>
    </el-card>
...
<script>
import { costInvoiceList, deletecostinvoice, updatecostinvoice2, costInvoiceGetList } from '@/api/CostInvoice'
...
export default {
  ...
  methods: {
    ...
    getlist() {
      ...
      costInvoiceGetList(this.getemplist).then(res => {
        if (res.data.ret === 200) {
    ...
    handleEdit(row) {
      ...
      costInvoiceList(parms).then(res => {
        if (res.data.ret === 200) {
