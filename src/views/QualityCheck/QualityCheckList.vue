<template>
  <div class="ERP-container">
    <el-card :body-style="	{ padding: '5px' }" class="box-card" style="margin-top: 5px" shadow="never">
      ...
      <el-input v-model="getemplist.title" :placeholder="$t('QualityCheck.title')" size="small" class="filter-item" clearable @keyup.enter.native="handleFilter"/>

      <el-input v-model="getemplist.checkNumber" :placeholder="$t('updates.zjbh')" size="small" class="filter-item" clearable @keyup.enter.native="handleFilter"/>

      <el-input v-model="reportPersonId" :placeholder="$t('QualityCheck.reportPersonId')" size="small" class="filter-item" clearable @clear="restFilter2" @keyup.enter.native="handleFilter" @focus="handlechooseStock"/>
      ...
      <el-popover
        ...
        size="small"
        trigger="click">
        <el-select v-model="getemplist.reportDeptId" size="small" placeholder="报检部门" clearable style="width: 40%;float: left;margin-left: 20px">
          <el-option
    ...
    <el-card :body-style="	{ padding: '6px' }" class="box-card" shadow="never">
      ...
      <!-- 表格导出操作 -->
      <!--      <el-button v-permission="['227-229-6']" v-waves :loading="downloadLoading" class="filter-item" style="width: 86px" @click="handleExport"> <svg-icon icon-class="daochu"/>{{ $t('public.export') }}</el-button>-->
      <!-- 打印操作 -->
      <el-button v-permission="['227-229-7']" v-waves class="filter-item2" size="small" icon="el-icon-printer" style="width: 86px" @click="handlePrint">{{ $t('public.print') }}</el-button>
      <!-- 新建操作 -->
      <el-button v-permission="['227-229-1']" v-waves class="filter-item2" size="small" icon="el-icon-plus" type="success" style="width: 86px" @click="handleAdd">{{ $t('public.add') }}</el-button>
    </el-card>
      ...
      <el-table
        ...
        <el-table-column :label="$t('public.id')" :resizable="false" fixed="left" align="center" min-width="150">
          <template slot-scope="scope">
            <span class="link-type" @click="handleDetail(scope.row)">{{ scope.row.checkNumber }}</span>
          </template>
          <detail-list :detailcontrol.sync="detailvisible" :detaildata.sync="personalForm" @rest="refreshlist"/>
        </el-table-column>
        <el-table-column :label="$t('QualityCheck.title')" :resizable="false" fixed="left" align="center" min-width="150">
          <template slot-scope="scope">
        ...
        <el-table-column :label="$t('public.actions')" :resizable="false" align="center" min-width="230">
          <template slot-scope="scope">
            <el-button v-permission2="['227-229-3', scope.row.createPersonId]" v-if="scope.row.judgeStat === 0" :title="$t('updates.xg')" type="primary" size="mini" icon="el-icon-edit" circle @click="handleEdit(scope.row)"/>
            <el-button v-permission2="['227-229-2', scope.row.createPersonId]" v-if="scope.row.judgeStat === 0" :title="$t('updates.sc')" size="mini" type="danger" icon="el-icon-delete" circle @click="handleDelete(scope.row)"/>
          </template>
        </el-table-column>
      </el-table>
      <!-- 列表结束 -->
...
<script>
import { qualitychecklist, updatequalitycheck2, deletequalitycheck, qualitycheckGetList } from '@/api/QualityCheck'
...
export default {
  ...
  methods: {
    ...
    getlist() {
      ...
      qualitycheckGetList(this.getemplist).then(res => {
        if (res.data.ret === 200) {
    ...
    handleEdit(row) {
      ...
      qualitychecklist(parms).then(res => {
        if (res.data.ret === 200) {
          ...
          if (res.data.data.content.list[0].checkType !== null) {
            this.personalForm.checkType = String(res.data.data.content.list[0].checkType)
          }
          if (res.data.data.content.list[0].checkMode !== null) {
            this.personalForm.checkMode = String(res.data.data.content.list[0].checkMode)
          }
        }
      })
    },
    ...
    handleReview(row) {
      ...
      this.$confirm(this.$t('prompt.qsh'), this.$t('prompt.sh'), {
        ...
        updatequalitycheck2(parms).then(res => {
          if (res.data.ret === 200) {
            this.$message({
              type: 'success',
    ...
    handleExport() {
      this.downloadLoading = true
        import('@/vendor/Export2Excel').then(excel => {
          ...
          const filterVal = ['id', 'QualityCheckName', 'QualityCheckShortName', 'typeName', 'regionName', 'buyerName', 'levelName', 'createName', 'createTime']
