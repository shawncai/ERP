<template>
  <div class="ERP-container">
    <el-card :body-style="	{ padding: '5px' }" class="box-card" style="margin-top: 5px" shadow="never">
      ...
      <el-input v-model="getemplist.title" :placeholder="$t('ProduceTask.title')" size="small" class="filter-item" clearable @keyup.enter.native="handleFilter"/>

      <el-input v-model="getemplist.taskNumber" :placeholder="$t('updates.djbh')" size="small" class="filter-item" clearable @keyup.enter.native="handleFilter"/>

      <el-input v-model="handlePersonId" :placeholder="$t('ProduceTask.handlePersonId')" size="small" class="filter-item" clearable @clear="restFilter" @keyup.enter.native="handleFilter" @focus="handlechooseStock"/>
      ...
      <el-popover
        ...
        size="small"
        trigger="click">
        <el-input v-model="workCenterId" size="small" style="width: 40%;float: left;margin-left: 20px" placeholder="工作中心" clearable @clear="restFilter3" @focus="workcenterchoose"/>
        ...
        <el-select v-model="getemplist.produceDeptId" size="small" placeholder="生产部门" clearable style="width: 40%;float: right;margin-right: 20px;margin-top: 20px">
          <el-option
        ...
        <el-select v-model="getemplist.processType" size="small" placeholder="加工类型" style="width: 40%;float: left;margin-left: 20px;margin-top: 20px">
    ...
    <el-card :body-style=" { padding: '6px'}" class="box-card" shadow="never">
      ...
      <!-- 表格导出操作 -->
      <el-button v-permission="['171-180-181-6']" v-waves :loading="downloadLoading" class="filter-item2" size="small" style="width: 86px" @click="handleExport"> <svg-icon icon-class="daochu"/>{{ $t('public.export') }}</el-button>
      <!-- 打印操作 -->
      <el-button v-permission="['171-180-181-7']" v-waves class="filter-item2" size="small" icon="el-icon-printer" style="width: 86px" @click="handlePrint">{{ $t('public.print') }}</el-button>
      <!-- 新建操作 -->
      <el-button v-permission="['171-180-181-1']" v-waves class="filter-item2" size="small" icon="el-icon-plus" type="success" style="width: 86px" @click="handleAdd">{{ $t('public.add') }}</el-button>
    </el-card>
...
<script>
import { producetasklist, deleteproducetask, updateproducetask2, producetaskGetList } from '@/api/ProduceTask'
...
export default {
  ...
  methods: {
    ...
    getlist() {
      ...
      producetaskGetList(this.getemplist).then(res => {
        if (res.data.ret === 200) {
    ...
    handleEdit(row) {
      ...
      producetasklist(parms).then(res => {
        if (res.data.ret === 200) {
