<template>
  <div class="ERP-container">
    ...
    <el-card class="box-card" style="margin-top: 15px">
      ...
      <!--修改开始=================================================-->
      <my-application :editcontrol.sync="editVisible" :editdata.sync="personalForm" @rest="refreshlist"/>
      <!--修改结束=================================================-->
    </el-card>
  </div>
</template>
...
import { searchapplication, deleteapplication } from '@/api/Storagemove'
...
export default {
  ...
  directives: { waves },
  components: { MyApplication, MyDepot, Pagination, MyRepository, MyAccept, MyCreate },
  filters: {
  ...
  methods: {
    ...
    getlist() {
      ...
      searchapplication(this.getemplist).then(res => {
        if (res.data.ret === 200) {
    ...
    handleCommand(command) {
      ...
      if (command === 'delete') {
        this.$confirm(this.$t('prompt.scts'), this.$t('prompt.ts'), {
          ...
          deleteapplication(ids, this.$store.getters.userId).then(res => {
    ...
    handleDelete(row) {
      this.$confirm(this.$t('prompt.scts'), this.$t('prompt.ts'), {
        ...
        deleteapplication(row.id, this.$store.getters.userId).then(res => {
          if (res.data.ret === 200 || res.data.ret === 100) {
            this.$notify({
              title: this.$t('prompt.sccg'),
              type: 'success',
    ...
    // 新增数据
    handleAdd() {
      this.$router.push('/Storagemove/AddMoveApplication')
    },
    // 导出
    handleExport() {
