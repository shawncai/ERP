      ...
      <el-card :body-style="{ padding: '5px' }" class="box-card" style="margin-top: 15px" shadow="never">
        ...
        <div class="container">
          <el-editable
            ...
            <el-editable-column :label="$t('Hmodule.wpbh')" fixed prop="productCode" align="center" min-width="150px"/>
            <el-editable-column :label="$t('Hmodule.wpmc')" fixed prop="productName" align="center" min-width="150px"/>
...
export default {
  ...
  components: { MyContract, MyRepository, MyLnquiry, MyDelivery, MyPlan, MyApply, MySupplier, MyDetail2, MyEmp },
  data() {
  ...
  methods: {
    ...
    // 保存操作
    async handlesave() {
      if (this.personalForm.deptId === null || this.personalForm.deptId === '' || this.personalForm.deptId === undefined) {
        this.$notify.error({
          title: 'wrong',
          message: '基本信息未填写完整',
          offset: 100
      ...
      if (this.personalForm.stockRepositoryId === null || this.personalForm.stockRepositoryId === '' || this.personalForm.stockRepositoryId === undefined) {
        this.$notify.error({
          title: 'wrong',
          message: '基本信息未填写完整',
          offset: 100
      ...
      if (this.personalForm.orderDate === null || this.personalForm.orderDate === '' || this.personalForm.orderDate === undefined) {
        this.$notify.error({
          title: 'wrong',
          message: '基本信息未填写完整',
          offset: 100
      ...
      if (this.personalForm.signPersonId === null || this.personalForm.signPersonId === '' || this.personalForm.signPersonId === undefined) {
        this.$notify.error({
          title: 'wrong',
          message: '基本信息未填写完整',
          offset: 100
      ...
      const arr2 = EnterDetail.map(item => { return { supplierId: item.supplierId } })
      console.log('arr2', arr2)
      for (let i = 0; i < arr2.length; i++) { // 遍历获取“前一个元素”，最后一个数不用获取，它本身已经被前面所有元素给排除过了
        for (let j = i + 1; j < arr2.length; j++) { // 遍历获取剩下的元素，“后一个元素”的起始索引就是“前一个元素”的索引+1
          if (arr2[i].supplierId === arr2[j].supplierId) { // 如果“前一个元素”与后面剩下的元素之一相同，那么就要删除后面的这个元素
            console.log('success')
            arr2.splice(j, 1)
            j--// 如果删除了这个元素，那么后面的元素索引值就会发生改变，所以这里的j需要-1
          }
        }
        arr2[i].countryId = this.personalForm.countryId
        arr2[i].createPersonId = this.personalForm.createPersonId
        arr2[i].sourceType = this.personalForm.sourceType
        arr2[i].stockRepositoryId = this.personalForm.stockRepositoryId
        arr2[i].stockPersonId = this.personalForm.stockPersonId
        arr2[i].deptId = this.personalForm.deptId
        arr2[i].orderDate = this.personalForm.orderDate
        arr2[i].signPersonId = this.personalForm.signPersonId
        arr2[i].isVat = this.personalForm.isVat
        arr2[i].currency = this.personalForm.currency
        arr2[i].exchangeRate = this.personalForm.exchangeRate
        arr2[i].repositoryId = this.personalForm.repositoryId
        arr2[i].regionId = this.personalForm.regionId
      }
      console.log('arr2', arr2)
      // const arr = EnterDetail.reduce(function(pre, current, index) {
      //   pre[current.supplierId] = pre[current.supplierId] || []
      //   pre[current.supplierId].push(current)
      //   return pre
      // }, {})
      console.log('EnterDetail', EnterDetail)
      for (let i = 0; i < arr2.length; i++) {
        const supplierId = arr2[i].supplierId
        console.log('supplierId', supplierId)
        const param = []
        for (let j = 0; j < EnterDetail.length; j++) {
          if (EnterDetail[j].supplierId === supplierId) {
            param.push(EnterDetail[j])
          }
        }
        arr2[i].detail = param
      }
    ...
    commit(val) {
      ...
      for (let i = 0; i < val2.length; i++) {
        ...
        let flag = 1
        const parms = JSON.stringify(Data)
        const parms2 = JSON.stringify(Data.detail)
        setTimeout(function() {
          flag = 2
          console.log(i)
          console.log(new Date())
          addstockorder(parms, parms2, val2[i]).then(res => {
            console.log(res)
          })
        }, i * 500)
        if (flag === 2) {
      ...
      //   const parms2 = JSON.stringify(Data.detail)
      //   setTimeout(function() {
      //      return addstockorder(parms, parms2, item)
      //   }, 2000)
      // }))
      this.restAllForm()
      this.$refs.editable.clear()
