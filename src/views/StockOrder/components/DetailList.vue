<template>
  <el-dialog :visible.sync="editVisible" :detailcontrol="detailcontrol" :detaildata="detaildata" :close-on-press-escape="false" :title="personalForm.orderNumber +$t('updates.cgddxq')" append-to-body width="1010px" class="edit" top="-10px" @close="$emit('update:detailcontrol', false)">
    <div id="printTest" >
      <!--基本信息-->
      ...
      <el-card class="box-card" style="margin-top: 15px" shadow="never">
        ...
        <div class="container">
          <el-editable
            ...
            <el-editable-column :label="$t('updates.sdsl')" prop="actualArrivalQuantity" align="center" />
          </el-editable>
        </div>
      </el-card>
...
import { updatestockorder2 } from '@/api/StockOrder'
...
export default {
    ...
    detaildata() {
      ...
      if (this.list2 !== '' && this.list2 !== undefined && this.list2 !== null) {
        for (let i = 0; i < this.list2.length; i++) {
          this.list2[i].taxRate2 = this.list2[i].taxRate
        }
      }
  ...
  methods: {
      ...
      if (hasPermission) {
        printJS({
          ...
          bottom: `<div>
                  ...
                  <div class="allmoneynum" style="border-left: 1px solid; height: 40px;display: flex;align-items: center;justify-content: center;padding-left: 20px">${this.personalForm.allIncludeTaxMoney}</div>
                  </div>
    ...
    handleMyReceipt2() {
      ...
      this.$store.dispatch('getstockOrder', this.detaildata)
