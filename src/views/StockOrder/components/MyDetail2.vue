<template>
  <el-dialog :visible.sync="productVisible" :control="control" :supp="supp" :close-on-press-escape="false" :title="$t('Hmodule.xzsp')" top="10px" append-to-body @close="$emit('update:control', false)">
    ...
    <span slot="footer" class="dialog-footer">
      <el-button v-waves type="success" style="text-align: center;" @click="handleAddTo2">{{ $t('Hmodule.sure') }}</el-button>
    </span>
    <el-dialog :visible.sync="proporcontrol" :close-on-press-escape="false" append-to-body title="批量采购" class="normal" width="440px" center>
      <el-form class="demo-ruleForm" style="margin: 0px 6%; width: 400px">
        <el-form-item label-width="100px;" style="width: 391px;">
          <div style="width: 100%; height: 103px;overflow: hidden;background: white;" >
            <el-form ref="addCategoryForm" :model="addForm" class="demo-ruleForm" style="margin: 0 auto; width: 400px">
              <el-form-item :label="$t('updates.shul')" label-width="120px">
                <el-input-number v-model="addForm.quantity" :precision="0" :controls="false" :step="1" :min="1" style="width: 150px"/>
              </el-form-item>
            </el-form>
            <el-button v-waves type="success" style="float: right;margin-top: 20px;margin-right: 44px;" @click="handleAddTo">{{ $t('Hmodule.sure') }}</el-button>
          </div>
        </el-form-item>
      </el-form>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    handleAddTo2() {
      if (this.moreaction === null || this.moreaction === '' || this.moreaction === undefined) {
        this.$notify.error({
          title: 'wrong',
          message: '请选择物品',
          offset: 100
    ...
    handleAddTo() {
      ...
      this.productVisible = false
      console.log('this.moreaction', this.moreaction.code)
      console.log('this.addForm.quantity', this.addForm.quantity)
      getStockInfoByProduct(this.moreaction.code, this.addForm.quantity).then(res => {
        if (res.data.ret === 200) {
          console.log('res', res.data.data.content)
          const productDetail = res.data.data.content.map(function(item) {
            return {
