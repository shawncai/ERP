...
<script>
import { empReceiptlist, deleteReceipt2, updateEmpReceipt } from '@/api/Inventorydamaged'
...
export default {
  ...
  filters: {
    sourceTypeFilter(status) {
      const statusMap = {
        1: _that.$t('update4.yuangongpeichagn'),
  ...
  methods: {
    ...
    handleReview4(row) {
      ...
      this.$confirm(this.$t('prompt.qfsp'), this.$t('prompt.fsp'), {
        ...
        updateEmpReceipt(parms).then(res => {
          if (res.data.ret === 200) {
    ...
    handleCommand(command) {
      ...
      if (command === 'delete') {
        this.$confirm(this.$t('prompt.scts'), this.$t('prompt.ts'), {
          ...
          deleteReceipt2(ids, this.$store.getters.userId).then(res => {
    ...
    handleDelete(row) {
      this.$confirm(this.$t('prompt.scts'), this.$t('prompt.ts'), {
        ...
        deleteReceipt2(row.id, this.$store.getters.userId).then(res => {
          if (res.data.ret === 200 || res.data.ret === 100) {
            this.$notify({
              title: this.$t('prompt.sccg'),
              type: 'success',
