...
import { updateEmpReceipt } from '@/api/Inventorydamaged'
...
import MyInventoryreceipt from './MyInventoryreceipt'
...
export default {
  ...
  watch: {
    ...
    editdata() {
      ...
      this.list = this.personalForm.empReceiptDetailVos
  ...
  methods: {
    ...
    handleEditok() {
      ...
      this.$refs.personalForm.validate((valid) => {
        if (valid) {
          updateEmpReceipt(parms, detailParms, this.$store.getters.userId).then(res => {
            if (res.data.ret === 200) {
              this.$notify({
