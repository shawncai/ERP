<template>
  <div class="ERP-container">
    ...
    <el-card :body-style=" { padding: '6px'}" class="box-card" shadow="never">
      ...
      <!-- 表格导出操作 -->
      <el-button v-permission="['171-190-406-6']" v-waves :loading="downloadLoading" class="filter-item2" size="small" style="width: 86px" @click="handleExport"> <svg-icon icon-class="daochu"/>{{ $t('public.export') }}</el-button>
      <!-- 打印操作 -->
      <el-button v-permission="['171-190-406-7']" v-waves class="filter-item2" size="small" icon="el-icon-printer" style="width: 86px" @click="handlePrint">{{ $t('public.print') }}</el-button>
      <!-- 新建操作 -->
      <el-button v-permission="['171-190-406-1']" v-waves class="filter-item2" size="small" icon="el-icon-plus" type="success" style="width: 86px" @click="handleAdd">{{ $t('public.add') }}</el-button>
    </el-card>
...
export default {
  ...
  components: { MyFactory, Pagination, MyDialog2, DetailList2 },
  filters: {
  ...
  methods: {
    ...
    handleReview4(row) {
      ...
      this.$confirm(this.$t('prompt.qfsp'), this.$t('prompt.fsp'), {
        ...
        updateFactoryAdjust(parms).then(res => {
          if (res.data.ret === 200) {
    ...
    factoryName(val) {
      ...
      this.getemplist.factoryId = val.id
    },
    // 工作中心focus事件
    workcenterchoose() {
    ...
    handleCommand(command) {
      ...
      if (command === 'delete') {
        this.$confirm(this.$t('prompt.scts'), this.$t('prompt.ts'), {
          ...
          deleteFactoryAdjust(ids, this.$store.getters.userId).then(res => {
    ...
    handleDelete(row) {
      this.$confirm(this.$t('prompt.scts'), this.$t('prompt.ts'), {
        ...
        deleteFactoryAdjust(row.id).then(res => {
          if (res.data.ret === 200 || res.data.ret === 100) {
            this.$notify({
              title: this.$t('prompt.sccg'),
              type: 'success',
    ...
    // 新增数据
    handleAdd() {
      this.$router.push('/OutSourcing/NewOutSouringAdjust')
    },
    // 导出
    handleExport() {
