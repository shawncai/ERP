<template>
  <div class="ERP-container">
    <el-card :body-style="{ padding: '10px' }" class="box-card" style="margin-top: 10px" shadow="never">
      ...
      <el-input v-model="getemplist.title" :placeholder="$t('StockInquiry.inquiryTitle')" size="mini" class="filter-item" clearable @keyup.enter.native="handleFilter"/>

      <el-input v-model="getemplist.number" :placeholder="$t('otherlanguage.xjdh')" size="mini" class="filter-item" clearable @keyup.enter.native="handleFilter"/>

      <el-input v-model="inquiryPersonId" :placeholder="$t('StockInquiry.inquiryPersonId')" size="mini" class="filter-item" clearable @keyup.enter.native="handleFilter" @clear="restFilter3" @focus="handlechooseStock"/>
      ...
      <el-popover
        ...
        width="500"
        trigger="click">
        <el-select v-model="getemplist.deptId" :placeholder="$t('updates.dept')" clearable size="mini" style="width: 40%;float: left;margin-left: 20px">
          <el-option
        ...
        <el-input v-model="supplierId" placeholder="供应商" size="mini" style="width: 40%;float: right;margin-right: 20px;" clearable @focus="handlechoose"/>
      ...
      <!-- 搜索按钮 -->
      <el-button v-waves class="filter-item" type="primary" size="mini" icon="el-icon-search" style="width: 86px" round @click="handleFilter">{{ $t('public.search') }}</el-button>
    </el-card>
    <el-card :body-style="{ padding: '10px' }" class="box-card" style="margin-top: 10px" shadow="never">
      <!-- 批量操作 -->
      ...
      <!-- 表格导出操作 -->
      <el-button v-permission="['104-110-6']" v-waves :loading="downloadLoading" size="mini" class="filter-item" style="width: 86px" @click="handleExport"> <svg-icon icon-class="daochu"/>{{ $t('public.export') }}</el-button>
      <!-- 打印操作 -->
      <el-button v-permission="['104-110-7']" v-waves class="filter-item" size="mini" icon="el-icon-printer" style="width: 86px" @click="handlePrint">{{ $t('public.print') }}</el-button>
      <!-- 新建操作 -->
      <el-button v-permission="['104-110-1']" v-waves class="filter-item" size="mini" icon="el-icon-plus" type="success" style="width: 86px" @click="handleAdd">{{ $t('public.add') }}</el-button>
    </el-card>
...
<script>
import { stockplanlist, updatestockInquiry2, deletestockInquiry, stockplanGetList } from '@/api/StockInquiry'
...
export default {
  ...
  methods: {
    ...
    getlist() {
      ...
      stockplanGetList(this.getemplist).then(res => {
        if (res.data.ret === 200) {
