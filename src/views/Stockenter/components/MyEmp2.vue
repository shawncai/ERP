<template>
  <el-dialog :visible.sync="employeeVisible" :entercontrol="entercontrol" :close-on-press-escape="false" :title="$t('Hmodule.xzyg')" top="10px" append-to-body @close="$emit('update:entercontrol', false)">
...
export default {
  ...
  methods: {
    ...
    handleConfirm() {
      this.employeeVisible = false
      this.$emit('enterName', this.choosedata)
    }
    // 仓库管理员选择结束
  }
}
