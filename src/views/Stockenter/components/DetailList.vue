...
import { updatestockenter, updatestockenter3 } from '@/api/Stockenter'
  ...
  methods: {
    ...
    handleReview() {
      this.$confirm(this.$t('prompt.qsh'), this.$t('prompt.sh'), {
        ...
        updatestockenter3(this.personalForm, 2, this.$store.getters.userId).then(res => {
          if (res.data.ret === 200) {
            this.$message({
              type: 'success',
        ...
        if (action === 'cancel') {
          ...
          this.$confirm('是否确认审核不通过？', 'Warning', {
            ...
            .then(() => {
              updatestockenter3(this.personalForm, 3, this.$store.getters.userId).then(res => {
                if (res.data.ret === 200) {
                  this.$message({
                    type: 'success',
                    message: this.$t('prompt.shcg')
