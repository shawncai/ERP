<template>
  <div class="ERP-container">
    ...
    <el-card :body-style="	{ padding: '5px' }" class="box-card" style="margin-top: 5px" shadow="never">
      ...
      <el-input v-model="getemplist.title" :placeholder="$t('WarehouseAdjust.title2')" size="small" class="filter-item" clearable @keyup.enter.native="handleFilter"/>

      <el-input v-model="getemplist.adjustNumber" :placeholder="$t('WarehouseAdjust.adjustNumber')" size="small" class="filter-item" clearable @keyup.enter.native="handleFilter"/>

      <el-input v-model="handlePersonId" :placeholder="$t('WarehouseAdjust.handlePersonId')" size="small" class="filter-item" clearable @focus="handlechooseDelivery"/>
      ...
      <el-popover
        ...
        size="small"
        trigger="click">
        <el-input v-model="adjustRepositoryId" size="small" placeholder="请选择入库仓库" style="float: left;margin-left: 20px" class="filter-item" clearable @focus="handlechooseRep"/>
        <my-repository :repositorycontrol.sync="repositorycontrol" @repositoryname="repositoryname"/>
        <el-select v-model="getemplist.enterDeptId" size="small" placeholder="请选择入库部门" clearable class="filter-item" style="float: left;margin-left: 20px">
          <el-option
    ...
    <el-card :body-style="	{ padding: '10px' }" class="box-card" shadow="never">
      ...
      <el-table
        ...
        <el-table-column
          ...
          fixed="left"
          align="center"/>
        <el-table-column :label="$t('WarehouseAdjust.id')" :resizable="false" fixed="left" align="center" min-width="150">
        ...
        <el-table-column :label="$t('WarehouseAdjust.title2')" :resizable="false" fixed="left" align="center" min-width="150">
          <template slot-scope="scope">
            <span>{{ scope.row.title }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$t('WarehouseAdjust.handlePersonId')" :resizable="false" align="center" min-width="150">
          <template slot-scope="scope">
            <span>{{ scope.row.handlePersonName }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$t('WarehouseAdjust.adjustDeptId')" :resizable="false" align="center" min-width="150">
          <template slot-scope="scope">
            <span>{{ scope.row.adjustDeptName }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$t('WarehouseAdjust.adjustRepositoryId')" :resizable="false" align="center" min-width="150">
          <template slot-scope="scope">
            <span>{{ scope.row.adjustRepositoryName }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$t('WarehouseAdjust.adjustDate')" :resizable="false" prop="endPersonName" align="center" min-width="150">
          <template slot-scope="scope">
            <span>{{ scope.row.adjustDate }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$t('WarehouseAdjust.adjustReason')" :resizable="false" prop="endPersonName" align="center" min-width="150">
          <template slot-scope="scope">
        ...
        <el-table-column :label="$t('public.actions')" :resizable="false" align="center" min-width="230">
          <template slot-scope="scope">
            <el-button v-permission="['131-163-166-3']" v-show="scope.row.judgeStat === 0" type="primary" size="mini" @click="handleEdit(scope.row)">{{ $t('public.edit') }}</el-button>
            <el-button v-permission="['131-163-166-76']" v-show="isReview4(scope.row)&&(scope.row.receiptStat === 1||scope.row.receiptStat === 2||scope.row.receiptStat === 3)" :title="$t('updates.fsp')" type="warning" size="mini" circle @click="handleReview4(scope.row)"><svg-icon icon-class="fanhui"/></el-button>
            ...
            <el-button v-permission="['131-163-166-2']" v-show="scope.row.judgeStat === 0" size="mini" type="danger" @click="handleDelete(scope.row)">{{ $t('public.delete') }}</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 列表结束 -->
...
<script>
import { adjustlist, deleteadjust, updateadjust2, adjustgetList } from '@/api/WarehouseAdjust'
  ...
  methods: {
    ...
    handleCommand(command) {
      ...
      if (command === 'delete') {
        this.$confirm(this.$t('prompt.scts'), this.$t('prompt.ts'), {
          ...
          deleteadjust(ids, this.$store.getters.userId).then(res => {
    ...
    handleDelete(row) {
      this.$confirm(this.$t('prompt.scts'), this.$t('prompt.ts'), {
        ...
        deleteadjust(row.id, this.$store.getters.userId).then(res => {
          if (res.data.ret === 200 || res.data.ret === 100) {
            this.$notify({
              title: this.$t('prompt.sccg'),
              type: 'success',
