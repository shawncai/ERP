...
export default {
  ...
  methods: {
    ...
    handleSuccess({ results, header }) {
      ...
      this.tableHeader = ['物料编码(必填)',	'条形码',	'商品名称(必填)',	'型号编号id',	'质保期(天数)',	'重量(保留两位小数)',	'体积(保留两位小数)',	'会员价',	'出厂价(必填)',	'批发价',	'销售价(必填)',	'最低价',	'档次级别编号(id)',	'基本计量单位编号(id)(必填)',	'销售计量单位编号(id)',	'库存计量单位编号(id)',	'生产计量单位编号(id)',	'来源(2生产3采购)',	'成本核算计价方式(1约当产量法，2定额成本法，3定额比例法)(必填)',	'上下架(1上架，2下架)',	'是否适用批次(1使用2不使用)(必填)',	'有效天数(必填)',	'采购周期(天)',	'保养周期(天数)',	'1可以作为赠品，2不可以',	'生产能力(日产量)',	'生产中心编号(id)',	'绩效分',	'积分', '种类id(必填)']
